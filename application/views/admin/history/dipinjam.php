					...
					foreach ($pinjam as $p) :
						...
						<tr>
							...
							<td>
								...
								$this->db->where('tbnote.id_peminjaman', $p->id_peminjaman);
								$data = $this->db->get();
								if ($data->num_rows() >= 1) :
									$photo = $data->row();
									?>
									<button type="button" class="btn btn-sm btn-outline-danger mx-auto" data-bs-toggle="modal" data-bs-target="#modalPhoto-<?=$p->id_peminjaman?>">
							...
							<!-- Modal -->
							<div class="modal fade" id="modalPhoto-<?=$p->id_peminjaman?>" tabindex="-1" aria-hidden="true">
								<div class="modal-dialog modal-dialog-centered" role="document">
									<div class="modal-content">
										<div class="modal-header">
											<h5 class="modal-title" id="modalCenterTitle" style="text-transform: capitalize;">Photo Bukti - <?=$p->id_peminjaman?></h5>
											<button
											type="button"
											class="btn-close"
											data-bs-dismiss="modal"
										...
										<div class="modal-footer">
											<button type="button" class="btn btn-sm btn-label-black" data-bs-dismiss="modal">
												Close
											</button> 
										</div>
									</div>
